...
| SC5 | The read-only boundary holds, including the `eligibility` write side effect, and no read reaches a live endpoint. | With read mode active: `admin trial <id> --update='…'` does not mutate through the read path and fails closed with a message that names the boundary; and `eligibility <id> …` returns its answer while the interest-signal insert is not persisted. A fetch stub records every call attempt; assert zero attempts reach a live PostgREST or edge-function endpoint across the read commands, and that `eligibility` attempts no `interest_signals` write. |
...
| SC8 | Stackless `--json` output matches the live-path shape for each read command. | Capture a `--json` baseline once from the live path for each read command and commit it as a fixture. In read mode with no stack, assert each command's `--json` output matches its committed baseline. Compare against the frozen artifact, not a live diff at verify time. |
