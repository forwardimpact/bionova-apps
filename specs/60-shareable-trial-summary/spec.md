...
| # | Claim | Verified by |
| --- | --- | --- |
| C1 | For `oncora-phase3`, the summary renders the trial name, a plain-language description of what it studies, an open-to-new-patients framing derived from the recruiting status, and its site cities — and does **not** render the protocol id, the sponsor, a therapeutic-area line, or the raw enrollment counts. | A test under `products/polaris/handlers/test/` asserting the rendered summary contains the trial name; contains a **non-empty plain-language study description that names the trial's condition in patient-readable terms** (composed from the condition and consent-summary prose — the seeded `oncora-phase3` condition is `lung-cancer`, rendered as lung cancer, never the `therapeutic_area` string `oncology`); contains its site cities and an open/recruiting-status phrase; contains none of the strings `BNV-ONC-2024-301`, `BioNova Therapeutics`, or `287 / 450`; and carries no rendered therapeutic-area field line. |
| C2 | The summary presents eligibility as a plain-language band (age range and required condition in readable terms) and never reproduces the inclusion/exclusion `custom[]` protocol strings (e.g. "Measurable disease per RECIST 1.1") or the `ECOG max` line. | A test under `products/polaris/handlers/test/` asserting the rendered summary **presents a plain-language eligibility band — the seeded age range (18–75) in patient-readable terms and the required condition named in plain language (lung cancer)** — and reproduces no `custom[]` string from the trial's `criteria` (e.g. "Measurable disease per RECIST 1.1") and contains no `ECOG` line. |
