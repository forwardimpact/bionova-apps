...
| `active` | Running but **closed** to new enrollment | `active_not_recruiting` |
...
**Decision 1 — `active` means `active_not_recruiting` only, not an umbrella.**
The issue asks whether `--status=active` should mean `active_not_recruiting`
alone or also cover `recruiting`/`not_yet_recruiting`. It means
`active_not_recruiting` **alone**. An umbrella `active` that also matched
...
returning the trials they return today; `--status=active` now returns the
...
| --- | --- | --- |
| SC1 | `--status=active` returns the `active_not_recruiting` trial, not an empty result. | handler test asserts `searchTrials({status:"active"})` returns the seed's one `active_not_recruiting` trial and a total of one matching trial; repro `just cli search --status=active` shows that trial |
| SC2 | `--status=active` does **not** surface any `recruiting` or `not_yet_recruiting` trial — the token is not an umbrella. | handler test asserts the `status:"active"` result excludes every `recruiting` and `not_yet_recruiting` trial |
