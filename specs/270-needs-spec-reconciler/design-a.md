...
| `scripts/needs-spec-reconciler.test.js`      | Fixture-driven unit tests over the pure classifier — covers SC 2–6, 9, 10, and the SC 16 paired open/merged fixture (one synthetic issue #9xx classified against two `fsSource` trees on the same run: `spec.md` ABSENT → RETAIN [SC 4b], then PRESENT with a `Serves issue #9xx` line → reconcile). The `fsSource(root)` seam makes the merged-vs-open hinge testable with no git or network.                                                              | `audit-gate.test.js`, `spec-design-watcher.test.js`          |
...
- **No abandoned-draft restore leg — prevented at source.** Because evidence is
  ...
  this durable and is being reconciled to match merged-only. This open-PR-only →
  RETAIN rule is exactly SC 16's fixture-backed hinge; #130 (linked only via open
  PR #171 for spec 150) is its illustrative live instance, not the anchor.
