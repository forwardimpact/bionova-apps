...
- **A spec parse error can never drop the label.** Reading an in-tree `spec.md` for references can fail. A parse error is a per-file, spec-level diagnostic. A file that fails to parse yields no positive links, so any issue that would have linked through it falls to fail-safe RETAIN. A parse error can never, on its own, cause an issue to drop `needs-spec`. Attributing a drop to "the issues linked through the file that failed to parse" is incoherent, since a file that did not parse has no known links. The reconciler records parse errors as a diagnostic (see the audit constraint), it does not act on them. The only per-issue outcomes are therefore reconcile (positive link) or RETAIN (no positive link); the internal reason enum that distinguishes RETAIN cases is a design concern.
...
- **Audit every decision, and surface parse errors.** The reconciler logs each decision — the issue, the resolved link, and the retain-vs-mutate outcome — so a silent false-drop or a linkage-spoof attempt is detectable after the fact. The same production audit output also surfaces every `spec.md` parse error it encountered (the `parseErrors` set), so an unreadable spec is visible rather than silently treated as "no link".
...
| 8 | Least-privilege token | the workflow declares exactly `contents: read` + `issues: write` under default-deny (agreeing with criterion 11); an `issues: write`-only grant would break `actions/checkout` |
...
| 15 | The production audit output surfaces every reconcile decision AND every spec parse error | the audit output records issue, resolved link, and outcome for each processed issue, and it also lists every `spec.md` that failed to parse (the `parseErrors` set); an unreadable spec is visible in the audit, not silently collapsed into "no link" |
