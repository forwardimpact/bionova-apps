...
| Positive-link detection by body reference | Linkage is established by a **body reference** to the issue in an existing spec's `spec.md` **merged to `main`**, never by issue-number match (`#NN` ≠ spec `NN`; e.g. #60 ≠ spec 60). An open or draft spec PR is not linkage. |
...
- **A malformed spec.md can never silently drop an issue.** Reading an in-tree `spec.md` for references can fail. A malformed or unreadable `spec.md` contributes zero links, so any issue that would have linked through it is RETAINED. It can never, on its own, cause an issue to drop `needs-spec`. Attributing a drop to "the issues linked through the file that failed to parse" is incoherent, since a file that did not parse has no known links. A malformed `spec.md` is never silent: it is surfaced in the run output as a diagnostic, attributed to no issue (see the audit constraint). The only per-issue outcomes are therefore reconcile (positive link) or RETAIN (no positive link); the reason detail that distinguishes RETAIN cases is a design concern.
...
- **Linkage evidence must be trusted-source, read from the in-tree state only.** The reference establishing a positive link must resolve from the in-tree `spec.md` content on `main` — never from any PR body, open OR merged. This in-tree-presence-on-`main` requirement IS the **merged-only** clearing convention from the Q3 #275/#277 coach ruling: `needs-spec` clears only once the linking spec's `spec.md` has merged to `main`, and draft- or open-PR linkage is insufficient. That naming is additive — it does not relax the stricter rule stated here that even a *merged* PR body is rejected; only the in-tree `spec.md` content on `main` counts as evidence. A merged spec PR's `spec.md` is already in the tree, so in-tree content is the single sufficient evidence source; this keeps `contents: read` of the checked-out tree enough, with zero PR-API dependency and no `pull-requests: read`. A PR body is attacker-forgeable: anyone able to open a PR could plant a spec-reference to an arbitrary issue and trigger a false drop of `needs-spec` + set `triaged`, silently suppressing legitimate spec work. A forged reference is confidently wrong, not ambiguous, so RETAIN does not catch it; bounding evidence to in-tree `main` content is the teeth behind the fail-safe. (The #129 citation seed lands in already-merged spec 210/60 and is therefore in-tree, so it satisfies this constraint.)
- **Merged `spec.md`, not an open PR, is what "a spec exists" means.** For the purpose of clearing `needs-spec`, a spec exists only when its `spec.md` has MERGED to `main`. An open or draft spec PR is mutable and abandonable — its `spec.md` lives on a branch, not in the tree — so it is not durable evidence a spec exists. The reconciler treats an issue whose only link is via an open/draft spec PR as **not linked → RETAIN**. This is the mutability-facing companion to the trusted-source constraint above (which is forgery-facing): distinct reasons, one converged rule — evidence resolves only from `spec.md` on `main`. Criterion 16 is the fixture-backed home for this rule and carries the illustrative live instance.
- **Explicit, default-deny permissions.** The workflow declares a `permissions:` block that is default-deny — exactly `contents: read` (to read `spec.md` from the checked-out tree) plus `issues: write` (label edits), and nothing else. Never `write-all`. Reading evidence from the tree rather than open-PR bodies means no `pull-requests: read` is needed. This block bounds only the default `GITHUB_TOKEN`; it cannot constrain a separately-minted token (for example a GitHub App installation token). So a default-deny block is necessary but not sufficient: if the label writes use a minted token, that token must be scoped AT MINT to the same two permissions and nothing more. The observable guarantee, however the token is obtained, is an effective scope of exactly `contents: read` + `issues: write` — the `contents: read` half is load-bearing, since a grant missing it breaks the `actions/checkout` step that reads `spec.md`.
...
- **Audit every decision, and surface malformed specs.** The reconciler logs each decision — the issue, the resolved link, and the retain-vs-mutate outcome — so a silent false-drop or a linkage-spoof attempt is detectable after the fact. The same run output also surfaces every malformed or unreadable `spec.md` as a diagnostic attributed to no issue, so an unreadable spec is visible rather than silently treated as contributing no links.
...
| 8 | Least-privilege on the token actually used to read `spec.md` and edit labels | the effective scope of that token is exactly `contents: read` + `issues: write` and no more, however the token is obtained — whether the default `GITHUB_TOKEN` or a separately-minted token scoped at mint. (Why a default-deny job block alone cannot bound a minted token: see the "Explicit, default-deny permissions" constraint.) |
...
| 10 | Linkage evidence resolves only from in-tree `spec.md` on `main` | a forged-reference fixture — a spec-reference to the issue that exists only in a PR body (open or merged), not in in-tree `spec.md` — does NOT trigger a drop; it RETAINs |
| 11 | The job `permissions:` block is default-deny | the workflow file declares a default-deny `permissions:` block of exactly `contents: read` + `issues: write`, no `write-all`. This is the plain block-declaration check; the effective-scope guarantee on whatever token performs the writes — including a minted token scoped at mint — is criterion 8's |
...
| 15 | The run output surfaces every reconcile decision AND, as a separate diagnostic, every malformed or unreadable `spec.md` | the output records issue, resolved link, and outcome for each processed issue; separately, it logs every `spec.md` that could not be read or parsed as a diagnostic attributed to no issue. A malformed spec is not a decision, so a run that only logged decisions would fail this criterion — the malformed spec must be visible, never silently collapsed into "no link" |
| 16 | An open/draft spec PR is not linkage — the label clears **iff** the linking spec's `spec.md` is present in the in-tree `main` root. This is the single fixture-backed home for the "open/draft spec PR → RETAIN" rule that criterion 4(b) and the "Merged `spec.md`, not an open PR" constraint reference | a **paired** fixture over one synthetic issue (e.g. #9xx), exercising both sides of the hinge on the same issue against two spec-tree states: (a) linking spec's `spec.md` ABSENT from the tree (models an open / not-yet-merged spec PR) → RETAIN, labels untouched; (b) that same `spec.md` PRESENT carrying a `Serves issue #9xx` line (models merged-to-`main`) → reconcile — `needs-spec` removed and `triaged` set. The pair makes the merged-vs-open hinge observable in one criterion — RETAIN on absence, clear on presence — which is what gives 16 its own teeth beyond the plain absent→RETAIN checks in 4(a)/10. (Illustrative real instance, not the anchor: #130, linked only via open PR #171 for spec 150 — an open-PR-only link, so RETAIN, full stop.) |
