...
| 16 | An open/draft spec PR is not linkage — only `spec.md` merged to `main` clears the label. This is the single fixture-backed home for the "open/draft spec PR → RETAIN" rule that criterion 4(b) and the "Merged `spec.md`, not an open PR" constraint reference | a constructed fixture — an issue whose only link is via an open (or draft) spec PR, with that spec's `spec.md` absent from `main` — is NOT cleared; it RETAINs. The fixture's PR stays open by construction, so the expected outcome is invariant RETAIN and never flips. (Illustrative real instance, not the verification anchor: #130, linked only via open PR #171 for spec 150 — its expected outcome would flip to "clear" if #171 ever merged, which is exactly why the anchor is the constructed fixture and not this live case.) |
