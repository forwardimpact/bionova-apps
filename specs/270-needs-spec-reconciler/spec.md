...
| 8 | Least-privilege on the token actually used to read `spec.md` and edit labels | the effective permissions of that token are exactly `contents: read` + `issues: write` and no more. If the token is minted (e.g. an App installation token) rather than the default `GITHUB_TOKEN`, its scope is set AT MINT to those two permissions — a default-deny job `permissions:` block alone does NOT satisfy this criterion, because that block bounds only the default `GITHUB_TOKEN` and cannot constrain a separately-minted token. A grant missing `contents: read` would break the `spec.md` read |
...
| 11 | The job `permissions:` block is default-deny (necessary), and any minted token is scoped to the same two permissions at mint (sufficient) | the workflow file declares a default-deny block of exactly `contents: read` + `issues: write`, no `write-all`. Because that block bounds only the default `GITHUB_TOKEN`, it is necessary but not sufficient: if the label writes use a minted token, its `permission-*` scope names exactly those two permissions and nothing more |
