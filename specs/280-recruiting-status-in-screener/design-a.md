...
| --- | --- | --- |
| Recruiting-status resolver | `products/polaris/handlers/src/recruiting-status.js` (new) | The pure `recruitingStatusLine` function above; the sole home of the fail-safe rule. A named export from the package index (`@bionova/polaris-handlers`), imported by the site exactly as `showTrial`/`checkEligibility` are. |
| Site type stub | `products/polaris/site/src/types/modules.d.ts` | Add `recruitingStatusLine` to the ambient `declare module "@bionova/polaris-handlers"` block. **The site does not read the package's JSDoc.** An ambient stub enumerates the package's exports (8 functions today), and TypeScript resolves the bare specifier to that stub, not the JS — so any symbol the site imports must be declared here or `tsc --noEmit` fails `TS2305`. One line; verified against the real stub, not assumed (see execution note in plan-a). |
