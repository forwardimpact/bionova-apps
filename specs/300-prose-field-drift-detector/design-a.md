...
- **Problem.** `story.dsl` (structured truth) and the generated patient-facing
  ...
  DIABPREV-201's shipped FAQ says the trial is enrolling while also saying it is
  not recruiting, and its consent summary says about 300 people will join, yet its
...
- **Success (SC1–SC6).** Reports DIABPREV-201's shipped drift — now on both its
  ...
  (SC4); the consent source lands as data — one more reader feeding the shared
  join and rule table, not a second detector (SC5); runs in the `check-seed` CI
  job so future drift fails before merge (SC6).
...
| ----------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------- |
| `readTrials(storySrc)`              | Scoped brace-depth reader over `story.dsl`: per `trial <id> { … }`, extract the declared `status` field — the sole charter family's structured operand. Not a general DSL parser — reads exactly the field the one rule needs; a spec-declared future criteria family (Explicitly excluded) would extend it.                                                                                                                                                                             | `→ { trials: {id: {status}}, parseErrors: [] }`          |
...
- **Family.** `{field, read(trial), contradicts(text, value)}` — source-agnostic.
  This spec ships exactly ONE family, `status`: `read` returns the trial `status`,
  `contradicts` maps it to recruiting-open via a fail-safe classifier then flags
  text asserting the opposite state (this also catches the self-contradicting FAQ,
  since one half opposes the structured field). The `{field, read, contradicts}`
  shape is deliberately open so a later family — `criteria-in-prose`,
  `field: "criteria"` — lands as one more data row once a live contradiction
  operand exists. That family is a spec-declared future extension (spec Explicitly
  excluded), not this charter: no SC exercises it, and its harder text-vs-rows
  comparator is out of scope here.
...
| Structured operand stays source-side: `story.dsl` via scoped brace-depth reader                                                                                      | The invariant is "shipped prose must not contradict the field it _derives from_" — the field is the vendored source; reads exactly `status` (the sole charter family's field), bounded and unit-testable.                                                                                                                                                                                                                                                                              | Reuse `fit-terrain`'s parser: a devDep bin that emits SQL, not a field API.                                                                                                                                                                            |
| Rule table parameterized by field family                                                                                                                             | One engine; the sole charter family is `status`. The one spec-declared later family, criteria-in-prose, is added as a data row once a live contradiction operand exists — a future extension, not this charter. The open shape is what lets that land as data, never a detector.                                                                                                                                                                                                       | A detector per field: duplicates the join + emit + diff — the ~80% restatement the #299/#127 fold exists to avoid.                                                                                                                                     |
...
- **SC5 holds — the consent source is data, not a second detector (confirmed).**
  Adding the consent surface is one reader call + a `source` tag feeding the same
  join / emit / diff — no parallel pipeline. Reviewed here per SC5. (The
  field-family axis is the same data-not-detector shape — a future `criteria`
  family would be one `FAMILIES` row — but that family is a spec-declared future
  extension, not this charter, so no SC exercises it.)
...
- **Consumed design-inputs.** issue-127-299-detector-fold (incl. the 2026-08-02
  ...
  sources, `status` the sole charter family with criteria-in-prose de-scoped to a
  spec-declared future extension (spec `1efc5dd`), correction terminates upstream,
  does _not_ close #127's staff-flag residual. spec-270 §1: audit output is a two-field
